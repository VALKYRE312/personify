...
async function handleSend() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#1F1518] text-white overflow-hidden">
      {/* TOP BAR */}
      <header className="relative z-50 w-full h-20 border-b border-[#3a2b30] bg-[#1A1114] flex items-center justify-between px-6">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => {
              // prefer history back so the user returns exactly where they came from
              navigate(-1);
            }}
            className="bg-[#F7A9B8] hover:bg-[#F4A7B3] text-black px-4 py-2 rounded-lg"
          >
            ← Back
          </button>
        </div>
