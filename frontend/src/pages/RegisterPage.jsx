...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // If backend returns the saved user:
const user = res.data.user || {
  ...
  birthday: form.birthday,
      ...
      // save user to localStorage so ProfilePage can read it
      localStorage.setItem("pa.user", JSON.stringify(user));

      // after sign up, send them to quiz
      navigate("/quiz");
    } catch (err) {
      console.error(err);
      const msg =
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-[#0f0d0b]">
        ...
        >
          ...
          <div className="flex gap-3 mb-5">
            <button className="flex-1 flex items-center justify-center gap-2 bg-[#2e2219] hover:bg-[#4a3626] p-2 rounded-lg border border-[#5c4433]">
